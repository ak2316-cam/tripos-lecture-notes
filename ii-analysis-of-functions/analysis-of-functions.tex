...
\begin{tikzpicture}[scale=1.0]
	...
	\begin{scope}[xshift=6.5cm]
		...
		\node[right] at (0.08,-0.28) {\scriptsize $-1$};
	\end{scope}
\end{tikzpicture}
\end{center}
\end{example}
...
\begin{proof}
	...
	For general $f \in H^s(\R^n)$, approximate: there are $f_m\in\Sch(\R^n)$ with $f_m\to f$ in $H^s$ and (after passing to a subsequence) almost everywhere. The sequence $(f_m)$ is Cauchy in $H^s$, hence by the estimate Cauchy in $C^k(\R^n)$ with the uniform norm on all derivatives up to order $k$; that space is complete, so $f_m\to f^*$ in $C^k$. Since also $f_m\to f$ a.e., $f = f^*$ a.e.
\end{proof}
