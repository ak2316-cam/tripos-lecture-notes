...
% --- The standard bump, precomputed: e^{1 - 1/(1-x^2)}, max 1 at 0 ---
...
\begin{tikzpicture}[scale=1.0]
	...
	\begin{scope}[xshift=8cm]
		...
		\draw[thick, color=blue] (1.0,0) circle (1.6);
		\node at (-2.0,1.1) {\small $L^1$};
		\node at (2.0,1.1) {\small $L^2$};
...
\begin{proof}
	Suppose $q < \infty$; the case $q = \infty$ is an easier variant. Write $|f|^r = |f|^{\theta r} |f|^{(1-\theta)r}$ and apply H\"older with the conjugate pair $\frac{p}{\theta r}$ and $\frac{q}{(1-\theta)r}$, which are indeed conjugate by the definition of $\theta$:
...
The integral certainly exists for every $x$ when, say, $f \in L^1(\R^n)$ and $g \in L^\infty(\R^n)$, or when $f, g \in C_c(\R^n)$. More generally, H\"older shows the integral converges absolutely when $f \in L^p$ and $g \in L^q$ for conjugate $p,q$. The following bookkeeping lemma records the algebraic properties.
...
\begin{example}[The standard bump]\label{ex:bump}
	Define $\psi : \R \to \R$ by
...
\begin{tikzpicture}[scale=1.0]
	\draw[<->] (-0.3,0) -- (7.3,0);
	\draw[<->] (0,-0.3) -- (0,2.6);
	% rough function (grey, dashed)
	\draw[thick, dashed, color=gray] (0.3,0) -- (0.3,1.4) -- (1.6,1.4) -- (1.6,0.5)
		-- (2.6,0.5) -- (3.4,1.8) -- (4.4,1.8) -- (4.4,0.8) -- (5.6,0.8)
		-- (5.6,1.5) -- (6.6,1.5) -- (6.6,0);
	\node[color=gray] at (0.9,2.15) {\small $g$};
	% mollified (blue, rounded)
	\draw[thick, color=blue, rounded corners=10pt] (0.3,0) -- (0.3,1.4) -- (1.6,1.4) -- (1.6,0.5)
		-- (2.6,0.5) -- (3.4,1.8) -- (4.4,1.8) -- (4.4,0.8) -- (5.6,0.8)
		-- (5.6,1.5) -- (6.6,1.5) -- (6.6,0);
	\node[color=blue] at (5.0,2.15) {\small $\phi_\varepsilon * g$};
\end{tikzpicture}
\end{center}
...
\begin{proof}
	...
	Consequently, if $x \in A_\lambda$ then either $M_{f-g}(x) > \lambda$ or $|f(x)-g(x)| > \lambda$. By Lemma \ref{lem:maximal} the first set has measure at most $C_n \varepsilon/\lambda$, and by Chebyshev's inequality the second has measure at most $\varepsilon/\lambda$. Hence
...
\begin{proof}
	...
	$$\left| \frac{G(x+h)-G(x)}{h} - g(x) \right| = \left|\frac 1h \int_x^{x+h} (g(t)-g(x))\dd t \right| \le \frac{2}{|B_h(x)|}\int_{B_h(x)}|g-g(x)| \to 0,$$
...
Lebesgue measure on $(\R^n, \mathcal{M})$ is regular, by Theorem \ref{thm:lebesgue}(iii). Now if $\mu$ is a finite regular measure on $\R^n$, then every $f \in C^0_c(\R^n;\R)$ is measurable and bounded, so
...
\begin{tikzpicture}[scale=1.0]
	...
	% compact convex A
	\draw[thick, color=blue] (5.0,0.9) ellipse (0.75 and 0.55);
	\node[color=blue] at (5.0,0.9) {$A$};
	% separating hyperplanes
	\draw[thick] (3.55,-0.6) -- (3.55,2.6);
	\draw[dashed] (3.95,-0.6) -- (3.95,2.6);
	\node[below] at (3.55,-0.65) {\small $\re \Lambda = \gamma_2$};
	\node[below] at (4.35,2.9) {\small $\re \Lambda = \gamma_1$};
	\draw[->] (4.35,2.75) -- (3.97,2.4);
\end{tikzpicture}
\end{center}
...
\begin{proof}
	...
	\begin{align*}
		...
		&\quad + |\Lambda(z')-\Lambda_{k,k}(z')| + |\Lambda(x')-\Lambda_{k,k}(x')| + |a||\Lambda(y')-\Lambda_{k,k}(y')| \\
...
\begin{tikzpicture}[scale=1.0]
	...
	\node[color=blue] at (-1.55,1.55) {$\ol B'$};
...
The Fourier transform converts differentiation into multiplication, and so converts constant-coefficient differential equations into algebra. That single fact is worth a whole course. What we want here is a clean functional-analytic account: on which spaces is the transform defined, what are its mapping properties, and can it be inverted.
...
\begin{proof}
	...
	$$\widehat{f*g}(\xi) = \int\!\!\int f(y)g(x-y)e^{-i(x\cdot\xi)}\dd y \dd x = \int f(y)e^{-i(y\cdot\xi)}\left( \int g(z)e^{-i(z\cdot\xi)}\dd z\right)\dd y = \hat f(\xi)\hat g(\xi). \qedhere$$
\end{proof}
...
\begin{proof}
	...
	$$\norm{f}_{L^1}=\int_{\R^n}\frac{|f|(1+|x|)^{n+1}}{(1+|x|)^{n+1}}\dd x \le C_n \sup_{x}\left( |f(x)|(1+|x|)^{n+1}\right), \quad C_n = \int_{\R^n}\frac{\dd x}{(1+|x|)^{n+1}}<\infty.$$
...
\begin{theorem}[Plancherel/Parseval]\label{thm:plancherel}
...
\begin{proof}[Proof sketch]
	For $f, g \in \Sch(\R^n)$ this is a computation: put $h = \ol{g}$, note $\hat{\ol g}(\xi) = \ol{\hat g(-\xi)}$, and apply the inversion theorem to $f * \check{\ol g}$ at the origin. The general case follows because $\Sch$ is dense in $L^1 \cap L^2$ for the norm $\norm{\cdot}_{L^1}+\norm{\cdot}_{L^2}$, by mollification and truncation as in Corollary \ref{cor:ccinftydense}.
\end{proof}
...
\begin{example}
	\begin{enumerate}[label=(\roman*)]
		...
		$$|T_f[\varphi]|=\left|\int(1+|x|)^{-N}f(x)\cdot(1+|x|)^N\varphi(x)\dd x\right|\le\left(\int\frac{|f|}{(1+|x|)^N}\right)\sup_x(1+|x|)^N|\varphi(x)|,$$
...
\begin{proof}
	...
	$$(D^\alpha(u_1*u_2))*\varphi=(u_1*u_2)*D^\alpha\varphi = u_1*(u_2*D^\alpha\varphi)=u_1*((D^\alpha u_2)*\varphi)=(u_1*D^\alpha u_2)*\varphi,$$
...
\begin{proof}
	Pick $\psi_0\in C^\infty(\R^n)$ supported in $Q^\circ$ with $\psi_0\ge0$ and $\psi_0\equiv1$ on $q$ (a smoothed indicator, built from the bump of Example \ref{ex:bump}). Set $s(x)=\sum_{y\in\Z^n}\psi_0(x-y)$; this sum is locally finite, so $s\in C^\infty$, it is $\Z^n$-periodic, and $s \ge 1$ since every $x$ has some translate in $q$. Then $\psi=\psi_0/s$ works.
...
\begin{proof}
	...
	For the bound, Lemma \ref{lem:Eprime} applied in $\Sch'$ gives $N,k,C$ with $|u[\varphi]|\le C\sup_{x,|\alpha|\le k}|(1+|x|)^ND^\alpha\varphi(x)|$, whence
