...
\begin{example}[Bolza]\label{ex:bolza}
	...
	That the infimum is nevertheless $0$ is the interesting half. Let $u_m$ be the sawtooth of slope $\pm1$ with $2m$ teeth: $u_m(0)=0$, $u_m' = \pm 1$ alternating on the intervals of length $1/(2m)$. Then $u_m' (x)^2= 1$ everywhere except at the finitely many corners, so the first term vanishes, while $|u_m|\le 1/(4m)$ pointwise, so
	$$S[u_m]=\int_0^1u_m^2\le\frac{1}{16m^2}\longrightarrow 0.$$
\begin{center}
\begin{tikzpicture}[scale=1.0]
	\draw[<->] (-0.4,0) -- (6.6,0);
	\draw[<->] (0,-0.9) -- (0,1.1);
	\node[below] at (6.0,-0.1) {\small $1$};
	\draw[dotted] (6.0,0) -- (6.0,0.75);
	% u_2 : 4 teeth of half-width 0.75, height 0.75
	\draw[thick, color=gray] (0,0) -- (0.75,0.75) -- (2.25,-0.75) -- (3.75,0.75)
		-- (5.25,-0.75) -- (6.0,0);
	\node[color=gray] at (0.95,0.95) {\small $u_2$};
	% u_6 : 12 teeth of half-width 0.25, height 0.25
	\draw[thick, color=blue] (0,0) -- (0.25,0.25) -- (0.75,-0.25) -- (1.25,0.25)
		-- (1.75,-0.25) -- (2.25,0.25) -- (2.75,-0.25) -- (3.25,0.25)
		-- (3.75,-0.25) -- (4.25,0.25) -- (4.75,-0.25) -- (5.25,0.25)
		-- (5.75,-0.25) -- (6.0,0);
	\node[color=blue] at (4.55,0.7) {\small $u_6$};
	\draw[color=blue, ->, shorten >=2pt] (4.55,0.55) -- (4.3,0.3);
\end{tikzpicture}
\end{center}
