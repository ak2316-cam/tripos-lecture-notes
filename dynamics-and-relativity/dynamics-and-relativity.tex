...
\newchapter{Newtonian Dynamics -- Basic Concepts}
...
\vocab{Equilibrium points} are points at which a particle can stay at rest for all time. The general condition for an equal point is
$$
V'(x) = 0,
$$
...
\begin{itemize}
	\item If $V''(x_0)>0$, then we have a local minimum of $V(x)$, and we have the harmonic oscillator equation. We can also see that the frequency of oscillation will be $\sqrt{V''(x_0)/m}$. We call this scenario a \vocab{stable equilibrium}.
	\item If $V''(x_0) < 0$, then we have a local maximum of $V(x)$. 
	This will have exponentially increasing and exponentially decaying solutions, and will almost always excite the exponentially increasing part. We have the growth rate $\gamma = \sqrt{-V''(x_0)/m}$. We call this an \vocab{unstable equilibrium}.
	\item If $V''(x_0) = 0$, then we need to go to higher terms in the Taylor series.
\end{itemize}

\begin{example}[Pendulum]
	Consider again a the same pendulum as before.
	If $\theta$ is the angle between the pendulum and the vertical, we get from Newton's second law that
	$$
	m l \dddot{\theta} = -mg \sin \theta,
	$$
	which comes from considering acceleration perpendicular to the string.
	\begin{center}
	...
	$$
			m l \dddot{\theta} = - \frac{\dd }{\dd \theta} (- mg \cos \theta),
	$$
	hence $E = T + V = \frac{1}{2}m l^2 \dot{\theta}^2 - mg l \cos \theta$. We can then note that $\frac{\dd E}{\dd t} = 0$.
...
Now considering the total word done by the force on the particle as it moves along a finite path:
\begin{center}
...
%Curve Lines [id:da129013690354006] 
\draw [color={rgb, 255:red, 208; green, 2; blue, 27 }  ,draw opacity=1 ]   (85,190.5) .. controls (109,139.5) and (191,196.5) .. (214,107.5) ;
%Shape: Circle [id:dp48673990705225145] 
\draw  [draw opacity=0][fill={rgb, 255:red, 0; green, 0; blue, 0 }  ,fill opacity=1 ] (159.6,160.9) .. controls (159.6,159.3) and (160.9,158) .. (162.5,158) .. controls (164.1,158) and (165.4,159.3) .. (165.4,160.9) .. controls (165.4,162.5) and (164.1,163.8) .. (162.5,163.8) .. controls (160.9,163.8) and (159.6,162.5) .. (159.6,160.9) -- cycle ;
%Shape: Circle [id:dp030662396621297106] 
\draw  [draw opacity=0][fill={rgb, 255:red, 0; green, 0; blue, 0 }  ,fill opacity=1 ] (211.1,107.5) .. controls (211.1,105.9) and (212.4,104.6) .. (214,104.6) .. controls (215.6,104.6) and (216.9,105.9) .. (216.9,107.5) .. controls (216.9,109.1) and (215.6,110.4) .. (214,110.4) .. controls (212.4,110.4) and (211.1,109.1) .. (211.1,107.5) -- cycle ;
%Shape: Circle [id:dp1936582741668741] 
\draw  [draw opacity=0][fill={rgb, 255:red, 0; green, 0; blue, 0 }  ,fill opacity=1 ] (82.1,190.5) .. controls (82.1,188.9) and (83.4,187.6) .. (85,187.6) .. controls (86.6,187.6) and (87.9,188.9) .. (87.9,190.5) .. controls (87.9,192.1) and (86.6,193.4) .. (85,193.4) .. controls (83.4,193.4) and (82.1,192.1) .. (82.1,190.5) -- cycle ;

% Text Node
\draw (87,193.5) node [anchor=north west][inner sep=0.75pt]    {$\mathbf{r}_{1} =\mathbf{r}( t_{1})$};
% Text Node
\draw (216,110.5) node [anchor=north west][inner sep=0.75pt]    {$\mathbf{r}_{2} =\mathbf{r}( t_{2})$};
% Text Node
\draw (164.5,163.9) node [anchor=north west][inner sep=0.75pt]    {$\mathbf{r}( t)$};
% Text Node
\draw (82,147.8) node [anchor=north west][inner sep=0.75pt]  [color={rgb, 255:red, 208; green, 2; blue, 27 }  ,opacity=1 ]  {$C$};
...
The total work is
$$
\int_{t_1}^{t_2} \vv{F} \cdot \vv{u} \; \dd t = \int_{t_1}^{t_2} \vv{F} \cdot \dot{\vv{r}} \; \dd t = \int_{\vv{r}(t_1)}^{\vv{r}(t_2)} \vv{F} \cdot \dd \vv{r},
$$
where we integrate over the curve $C$.
We can write this in terms of components, with
$$
	\int_{t_1}^{t_2} F_x \dd x + F_y \dd y + F_z \dd z,
$$
where $\vv{F} = (F_x, F_y, F_z)$.
...
\begin{definition}[Conservative Force Field]
	A \vocab{conservative force field} is one such that $\vv{F}(\vv{r}) = - \nabla V(\vv{r})$ for some function $V(\vv{r})$.
\end{definition}
