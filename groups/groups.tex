...
\end{proposition}
\begin{proof}
	If $aH = a'H$, then $a \in a'H$, so $a = a' h_1$ for some $h_1 \in H$. Also if $bH = b'H$, we have $b = h_2 b'$ for $h_2 \in H$. Thus
	$ab = a' h_1 h_2 b'$. But {\color{red} To Be Continued}
\end{proof}



% In a previous chapter, we looked at how to recognize when there was some homomorphism from a group $G$ to another group $H$. 
% In particular we had a criterion for some homormo the criterion for that homormorphism t

% In a previous chapter, we looked at how to recognize when there is a homomorphism from some group $G$ to another group $H$, that is, when a group is an 

% Let's begin by considering some familiar subgroup $n\Z \normal \Z$. The cosets of this group are of the form
% $
% 0 + n\Z, \ 1 + n\Z, \ 2 + n\Z, \dots,\ (n - 1) + n \Z.
% $
% These cosets, although they are subsets of $\Z$, they really behave a lot like elements of the group $\Z/n\Z = \{0, 1, \dots, n -1\}$ if we try to `add' them, which we can try define as .
% $$
% 	(k + n\Z)  + (m + n\Z ) = (k + m) + n\Z.
% $$

% If we had some general subgroup $H \leq G$, we could try and do the same, to define
% $
% g_1 H \cdot g_2 H = g_1 g_2 H.
% $
% This may not be well-defined, however -- it can depend on the choice of $g_1$ and $g_2$, the coset representatives.
 
% To guarantee this is well defined (so that this set of cosets is a group), taking different coset representatives $g_1'$ and $g_2'$, we'd need $g_1' H = g_1 H$ and $g_2' H = g_2 H$. Then we'd need $g_1' g_2' H = g_1 g_2 H$.

% If $g_1'H = g_1 H$ and $g_2' H = g_2 H$, then $g_1' = g_1 h_1$ and $g_2' = g_2 h_2$ for some $h_1, h_2 \in H$. So $g_1'$

% There may be some problems with this, however. If we imagine our group $G$ to be split up into a series of cosets, then what we are doing is taking two coset representatives and multiplying them, and we will end up getting the coset corresponding to $g_1 g_2 H$. 
