...
With the aim of getting the simplest construction possible, we can `stick them together' in an `abelian way': by defining a group operation on the product $G \times H = \{ (g, h)  \; : \; g \in G, h \in H\}$ (a set of ordered pairs).
...
% 	\begin{align*}
% 		\phi((h_1, k_1) \cdot (h_2, k_2)) = \phi(h1 h2, )
% 	\end{align*}
% \end{proof}
...
\end{itemize}
Indeed these are just two descriptions of the same thing. The convention is often to refer to $H \times \{e_k\}$ and $\{e_H\} \times K$ as $H$ and $K$ respectively.

{\color{red} TODO: Go over this a bit more, and give an actually useful example}
...
\begin{definition}[Cyclic]
	If $G$ is a group and there is some $a \in G$ such that $\langle a \rangle$ is a generator of $G$, then we say $G$ is \vocab{cyclic}. 
\end{definition}
...
\begin{theorem}[Cyclic Groups are Isomorphic]
	A cyclic group $G$ is isomorphic to $C_n$ for some $n \in \N$.
\end{theorem}
\begin{proof}
	Let $G = \langle b \rangle$. Suppose first that there exists $n > 0$ such that $b^n = e$. Take the smallest such value of $n$.
	Define $\phi: C_n = \{e, a, \dots, a^{n - 1}\} \rightarrow G$ by $\phi(a^k) = b^k$ for all $0 \leq k \leq n - 1$. 
	Then for any two elements $a^j, a^k \in C_n$, if $j + k < n$ $\phi(a^j a^k) = \phi(a^{j + k}) = b^{j + k} = b^j b^k = \phi(a^j) \phi(a^k)$, and if $j + k \geq n$, $\phi(a^j a^k) = \phi(a^{j + k - n}) = b^{j + k - n} = b^{j + k} b^{-n} = b^j b^k = \phi(a^j) \phi(a^k)$, so $\phi$ is a homomorphism.
	Now since $b^n = e \in G$, all elements of $G$can be written as $b^k$, where $0 \leq k < n$, so $\phi$ is surjective.
	$\phi$ is also injective, since $\phi(a^k) = e \implies b^k = e$ and $k = 0$. So $\phi$ is an isomophism, and $G \cong C_n$.

	If no such $n$ exists, we define $\phi: \Z \rightarrow G$ by $\phi(k) = b^k$. Then $\phi(k + m) = b^{k + m} = b^k b^m = \phi(k) \phi(m)$, so $\phi$ is a homomorphism. Clearly $\phi$ is surjective, where $G = \langle b \rangle$. Now suppose $m \in \kernel(\phi)$. Then $\phi(m) = b^m = e$ and $\phi(-m) = b^{-m} = (b^m)^{-1} = e$, so if $m \neq 0$, we would get a contradiction to the fact that there does not exist $n > 0$ such that $b^n = e$. So $m = 0$ and thus $\kernel(\phi) = \{0\}$, so $\phi$ is injective, and thus $\phi$ is an isomorphism, that is, $G \cong \Z$.
\end{proof}
...
We will now look at some groups related to geometry.
...
So what elements make up this group? Clearly we have $n$ rotations of angles $\frac{2\pi k}{n}$, for $0 \leq k < n$. We also have $n$ reflections (noting that when $n$ is odd, we have $n$ reflections in axis through the center and each of the vertices, and when $n$ is even they go through )
