...
\tikzset{
  sta/.style={circle, draw, thick, minimum size=9mm, inner sep=0.5pt, font=\small},
...
\begin{example}\label{ex:oddas}
	...
	$$P_1 = \{S \to aSa,\, S \to a\}, \quad P_2 = \{S \to Saa,\, S \to a\}, \quad P_3 = \{S \to aaS,\, S \to aaSaa,\, S \to a\}$$
...
\begin{tikzpicture}[scale=1]
	\draw[blob] (-4.1,-2.5) rectangle (4.1,2.5);
	\draw[thick] (0,-0.15) ellipse (3.5 and 2.0);
	\draw[thick] (-0.35,-0.15) ellipse (2.6 and 1.5);
	\draw[thick] (-0.75,-0.15) ellipse (1.7 and 1.0);
	\node at (0,2.1) {\small type $0$};
	\node at (2.1,1.15) {\small type $1$};
	\node at (1.1,0.75) {\small type $2$};
	\node at (-0.75,-0.15) {\small type $3$};
	\node at (-2.9,2.15) {\small all languages};
	\node[font=\footnotesize] at (0.05,-1.15) {$\{a^nb^nc^n\}$};
	\node[font=\footnotesize] at (-0.72,-0.72) {$\{0^k1^k\}$};
	\node[font=\footnotesize] at (2.35,-1.45) {$\KK$};
	\node[font=\footnotesize] at (-3.35,-2.05) {$\WW \setminus \KK$};
\end{tikzpicture}
\end{center}
...
\begin{example}\label{ex:evenas}
	...
	\begin{tikzpicture}
		\node[sta] (q0) at (0,0) {$q_0$};
		\node[acc] (E) at (2.6,1.2) {$E$};
		\node[sta] (O) at (2.6,-1.2) {$O$};
		\initarr{q0}{(-1.1,0)}
		\draw[tra] (q0) -- node[tlab, above left] {$b$} (E);
		\draw[tra] (q0) -- node[tlab, below left] {$a$} (O);
		\draw[tra] (E) to[out=-70, in=70] node[tlab, right] {$a$} (O);
		\draw[tra] (O) to[out=110, in=-110] node[tlab, left] {$a$} (E);
		\draw[tra] (E) to[out=60, in=120, looseness=6] node[tlab, above] {$b$} (E);
		\draw[tra] (O) to[out=-60, in=-120, looseness=6] node[tlab, below] {$b$} (O);
	\end{tikzpicture}
	\end{center}
...
\begin{example}\label{ex:nfaendab}
	...
	\begin{tikzpicture}
		...
		\draw[tra] (s) to[out=120, in=60, looseness=5] node[tlab, above] {$a,b$} (s);
	\end{tikzpicture}
	\end{center}
...
\begin{proof}
	...
	$$D = (\Sigma,\; \power(Q),\; \Delta,\; \{q_0\},\; \mathcal{F}), \qquad \Delta(X, a) = \bigcup_{q \in X} \delta(q, a), \qquad \mathcal{F} = \{X \subseteq Q \mid X \cap F \neq \varnothing\}.$$
...
\begin{example}\label{ex:subsetworked}
	...
	\begin{tikzpicture}
		...
		\node[font=\small] at (2.4,1.5) {$D$};
		\node[sta] (X) at (0,0) {$X$};
		\node[sta] (Y) at (2.4,0) {$Y$};
		\node[acc] (Z) at (4.8,0) {$Z$};
		\initarr{X}{(-1.0,0)}
		\draw[tra] (X) -- node[tlab, above] {$a$} (Y);
		\draw[tra] (Y) -- node[tlab, above] {$b$} (Z);
		\draw[tra] (X) to[out=120, in=60, looseness=5] node[tlab, above] {$b$} (X);
		\draw[tra] (Y) to[out=120, in=60, looseness=5] node[tlab, above] {$a$} (Y);
		\draw[tra] (Z) to[out=-30, in=30, looseness=5] node[tlab, right] {$a$} (Z);
		\draw[tra] (Z) to[out=-120, in=-60] node[tlab, below] {$b$} (X);
		\draw[tra] (Z) to[out=-150, in=-30] node[tlab, below] {} (Y);
		\end{scope}
	\end{tikzpicture}
	\end{center}
...
\begin{example}\label{ex:needstates}
	Fix $n > 0$ and let $L = \{0^n w \mid w \in \WW\}$, the words beginning with $n$ zeroes. This is regular: take $V = \{S, X_0, \dots, X_{n-1}\}$ and
	\begin{align*}
...
\begin{proof}
	Let $L = \lang(D)$ with $D = (\Sigma, Q, \delta, q_0, F)$. We may assume that $\delta(q,a) \neq q_0$ for all $q, a$: if not, add a new start state $q_0'$ with $\delta(q_0', a) = \delta(q_0,a)$ and make $q_0$ an ordinary state, accepting or not according to whether some word takes $q_0'$ back to it and lands in $F$ -- more simply, take a disjoint copy $q_0'$ of $q_0$ as the new start state, put $\delta(q_0',a) = \delta(q_0,a)$, and let $q_0$ inherit its accepting status from whether $q_0 \in F$, which is to say it is not accepting. Then no transition enters $q_0'$, and the language is unchanged since $\hat\delta(q_0', w) = \hat\delta(q_0, w)$ for $w \neq \eps$.
...
\begin{definition}[Regular expression]
	...
	$$\lang(R+S) = \lang(R)\cup\lang(S), \quad \lang(RS) = \lang(R)\lang(S), \quad \lang(R^\star) = \lang(R)^\star, \quad \lang(R^+) = \lang(R)^+.$$
\end{definition}
...
\begin{proof}
	...
	$$R^k_{i,j} = \{w \in \WW \mid \hat\delta(q_i, w) = q_j \text{ and every state strictly inside the run lies in } \{q_0, \dots, q_{k-1}\}\},$$
...
\begin{center}
\begin{tikzpicture}
	\draw[blob] (-0.4,-2.3) rectangle (4.4,1.5);
	\node[font=\footnotesize] at (0.35,1.15) {$\WW$};
	\draw[thick] (1.1,0.45) ellipse (1.15 and 0.72);
	\draw[thick] (3.1,0.45) ellipse (1.05 and 0.72);
	\draw[thick] (2.0,-1.35) ellipse (1.5 and 0.72);
	\node[font=\footnotesize] at (1.1,0.45) {$[\eps]_L$};
	\node[font=\footnotesize] at (3.1,0.45) {$[u]_L$};
	\node[font=\footnotesize] at (2.0,-1.35) {$[v]_L$};
	\node[sta] (s0) at (7.1,0.9) {};
	\node[sta] (s1) at (9.3,0.9) {};
	\node[acc] (s2) at (8.2,-1.3) {};
	\initarr{s0}{(-1.0,0)}
	\draw[tra] (2.25,0.45) -- node[tlab,above,pos=0.45] {} (2.05,0.45);
	\draw[tra, dashed, shorten >=3pt] (2.25,0.6) -- (5.9,0.9);
	\draw[tra, dashed, shorten >=3pt] (4.15,0.45) -- (6.35,0.85);
	\draw[tra, dashed, shorten >=3pt] (3.5,-1.35) -- (7.2,-1.3);
	\node[font=\footnotesize] at (6.0,-1.75) {states of $D_L$};
\end{tikzpicture}
\end{center}
...
\begin{example}
	...
	\end{tikzpicture}
	\end{center}
	Its height is $4$, its leaves are $01, 10, 20, 000, 0010$, and the branch to $0010$ is $\eps, 0, 00, 001, 0010$.
\end{example}
...
\begin{proof}
	...
	Both are context-free: $L_0$ is the concatenation of $\{a^nb^n\}$ and $\{c^k\}$, and $L_1$ is the concatenation of $\{a^k\}$ and $\{b^nc^n\}$, and each factor is context-free by the grammar of Example~\ref{ex:parsetree} or by a regular grammar. But $L_0 \cap L_1 = \{a^nb^nc^n \mid n > 0\}$, which is not context-free by Example~\ref{ex:anbncn}.
...
\begin{tikzpicture}
	...
	\foreach \i/\s in {0/a, 1/a, 2/b, 3/b, 4/a, 5/b, 6/{}} {
		\draw[thick] (\i*0.75, 2.3) rectangle (\i*0.75+0.75, 3.0);
		\node[font=\small] at (\i*0.75+0.375, 2.65) {$\s$};
	}
	\node[font=\footnotesize, anchor=east] at (-0.2,2.65) {input};
	\draw[tra] (1.875,2.25) -- (1.875,1.35);
	\node[font=\footnotesize, anchor=west] at (1.95,1.8) {reading head};
	% control
	\draw[blob, fill=none] (0.4,0.35) rectangle (3.35,1.3);
	\node[font=\small] at (1.875,0.82) {finite control, state $q$};
	% stack
	\foreach \i/\s in {0/{\$}, 1/{Z}, 2/{Z}, 3/{Y}} {
		\draw[thick] (5.6, \i*0.62) rectangle (6.5, \i*0.62+0.62);
		\node[font=\small] at (6.05, \i*0.62+0.31) {$\s$};
	}
	\draw[thick, dashed] (5.6,-0.45) rectangle (6.5,0.0);
	\node[font=\footnotesize, anchor=west] at (6.65,2.17) {top of stack};
	\draw[tra] (6.6,2.05) -- (6.35,1.95);
	\node[font=\footnotesize, anchor=west] at (6.65,-0.22) {bottom};
	% arrows between control and stack
	\draw[tra] (3.45,1.05) to[out=0,in=180] node[tlab, above] {\footnotesize push/pop} (5.5,1.55);
	\draw[tra] (5.5,0.6) to[out=180,in=0] node[tlab, below] {\footnotesize top symbol} (3.45,0.6);
\end{tikzpicture}
\end{center}
...
\begin{center}
\begin{tikzpicture}
	\node[sta] (q) at (0,1.0) {$q$};
	\node[font=\footnotesize, anchor=east] at (-0.75,1.0) {state};
	% registers
	\foreach \k/\x in {0/2.6, 1/4.4, 2/6.2} {
		\node[font=\footnotesize] at (\x,2.35) {register $\k$};
	}
	% register 0: word "aba"
	\foreach \i/\s in {0/a, 1/b, 2/a} { \draw[reg] (2.6,\i*0.6+0.2) rectangle ++(0.9,0.6); \node[font=\small] at (3.05,\i*0.6+0.5) {$\s$}; }
	% register 1: word "bb"
	\foreach \i/\s in {0/b, 1/b} { \draw[reg] (4.4,\i*0.6+0.2) rectangle ++(0.9,0.6); \node[font=\small] at (4.85,\i*0.6+0.5) {$\s$}; }
	% register 2: empty
	\draw[reg, dashed] (6.2,0.2) rectangle ++(0.9,0.6);
	\node[font=\footnotesize] at (6.65,-0.15) {$\eps$};
	\node[font=\footnotesize] at (3.05,-0.15) {$aba$};
	\node[font=\footnotesize] at (4.85,-0.15) {$bb$};
	\draw[tra] (2.15,2.05) -- (2.85,2.05);
	\node[font=\footnotesize, anchor=east] at (2.1,2.05) {top};
	\draw[thick, dotted] (3.5,2.0) -- (7.1,2.0);
\end{tikzpicture}
\end{center}
...
\begin{example}\label{ex:rmflow}
	...
	\begin{tikzpicture}
		...
		\draw[tra] (q2) to[out=60,in=0, looseness=1.3] node[tlab, right] {pop} (0.5,0.55) to[out=180,in=90] (qS);
	\end{tikzpicture}
	\end{center}
...
\begin{lemma}\label{lem:stepfunction}
	The function
	$$h(w,u,v) = \begin{cases} \code\big(C(\#v, M, \vv w)\big) & \text{if } w = \code(M) \text{ and } u = \code(\vv w) \text{ for some } M, \vv w, \\ \uparrow & \text{otherwise}\end{cases}$$
	is computable.
\end{lemma}
...
\begin{proof}
	Failure of complement is Corollary~\ref{cor:sigmanotcomplement}; since $\WW \setminus A = \WW \setminus A$ is a difference, difference fails too.
...
$$\KK \leq_m A \implies A \text{ is not computable}, \qquad \WW\setminus\KK \leq_m A \implies A \text{ is not computably enumerable}.$$
...
\begin{tikzpicture}[every node/.style={font=\footnotesize}]
	...
	\node (k) at (-2.1,1.5) {$\KK$};
	\node (nk) at (2.1,1.5) {$\WW\setminus\KK$};
	\node (j) at (0,3.0) {$\KK \oplus (\WW\setminus\KK)$};
	\draw[thick] (bot) -- (k); \draw[thick] (bot) -- (nk);
	\draw[thick] (k) -- (j); \draw[thick] (nk) -- (j);
	\node[anchor=west] at (-3.9,1.5) {};
	\node[anchor=east, align=right] at (-2.6,1.5) {$\Sigma_1$-complete};
	\node[anchor=west, align=left] at (2.6,1.5) {$\Pi_1$-complete};
	\node[anchor=south] at (0,3.35) {neither $\Sigma_1$ nor $\Pi_1$};
\end{tikzpicture}
\end{center}
...
\begin{example}
	...
	$$\Emp = \{v \mid W_v = \varnothing\}, \quad \Fine = \{v \mid W_v \text{ is finite}\}, \quad \Inf = \{v \mid W_v \text{ is infinite}\}, \quad \Tot = \{v \mid W_v = \WW\}.$$
...
$$\text{regular} \;\subseteq\; \text{context-free} \;\subseteq\; \text{context-sensitive} \;\subseteq\; \text{computable} \;\subseteq\; \text{computably enumerable} \;\subseteq\; \power(\WW).$$

\begin{center}
\begin{tikzpicture}
	\draw[blob] (-5.6,-3.15) rectangle (5.6,3.15);
	\draw[thick] (-0.55,0) ellipse (4.85 and 2.65);
	\draw[thick] (-0.95,0) ellipse (4.05 and 2.25);
	\draw[thick] (-1.35,0) ellipse (3.25 and 1.85);
	\draw[thick] (-1.75,0) ellipse (2.45 and 1.45);
	\draw[thick] (-2.15,0) ellipse (1.6 and 1.0);
	\node[font=\small] at (-2.15,0.15) {regular};
	\node[font=\footnotesize] at (-2.15,-0.35) {(type $3$)};
	\node[font=\small] at (0.6,0.9) {context-free (type $2$)};
	\node[font=\small] at (1.35,1.55) {context-sensitive (type $1$)};
	\node[font=\small] at (2.2,2.0) {computable ($\Delta_1$)};
	\node[font=\small] at (2.55,2.4) {computably enumerable (type $0$, $\Sigma_1$)};
	\node[font=\small] at (-3.6,2.75) {all languages};
	\node[font=\footnotesize] at (-1.2,-1.2) {$\{0^k1^k\}$};
	\node[font=\footnotesize] at (-0.55,-1.62) {$\{a^nb^nc^n\}$};
	\node[font=\footnotesize] at (0.1,-2.03) {$\mathbb{D}$};
	\node[font=\footnotesize] at (0.75,-2.42) {$\KK$};
	\node[font=\footnotesize] at (2.1,-2.85) {$\WW\setminus\KK$};
\end{tikzpicture}
\end{center}
