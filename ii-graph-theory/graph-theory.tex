...
Now let's look at another question, which is what does a typical graph $G$ sampled from $\GG(n, p)$ look like? What does its structure look like as $p$ varies from 0 to 1? As a first example, we will consider the following more precise question:
...
\begin{theorem}
	Let $G$ be sampled from $\GG(n, p)$, where $p = p(n)$, and let $t \in \N$. Then
	$$
	\PP(G \text{ contains a $K_t$}) \longrightarrow 1 \quad \text{if} \quad p(n)n^{\frac{2}{t - 1}} \longrightarrow \infty \quad \text{as} \quad n \longrightarrow \infty.
	$$
	Moreover, $\PP(\text{$G$ contains $K_t$}) \rightarrow 0$ if $p(n) n^{\frac{2}{t - 1}} \rightarrow 0$ as $n \rightarrow\infty$.
\end{theorem}
