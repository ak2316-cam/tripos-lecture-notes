...
% For some complex number $z = x + yi$, the quantity $x - yi$, the \emph{conjugate} of $z$, is frequently of use.

% \begin{definition}[Complex Conjugate]
% 	For a complex number $z = x + yi$, we define the \vocab{complex conjugate} of $z$ to be $\overline{z} = z^* = x - yi$.	
% \end{definition}

% Complex conjugates allow us to extract the real and imaginary parts of complex numbers, as
% $$
% \re(z) = \frac{z + \overline{z}}{2}, \quad \text{and} \quad \im(z) = \frac{z - \overline{z}}{2i}.
% $$
% This directly implies that $z \in \R$ if and only if $z = \overline{z}$, which is a useful test for a number being real.
% From the definitions of addition and multiplication, we also have that $\overline{z + w} = \overline{z} + \overline{w}$, and $\overline{zw} = \overline{z} \cdot \overline{w}$.  

% We can see that the operations of $+$ and $\times$ are commutative, and associative. 
% As we also have inverses, we can use some terminology from `Groups' to state summaries this.
% \begin{proposition}[$\C$ is Field]
% 	We have that
% 	\begin{itemize}
% 		\item $\C$ with the operation $+$ is an abelian group with identity $0$;
% 		\item $\C \backslash \{0\}$ with the operation $\times$ is an abelian group with identity $1$;
% 		\item Multiplication is distributive over addition in $\C$, with $z\times(w_1 + w_2) = z\times w_1 + z \times w_2$ for $z, w_1, w_2 \in \C$,
% 	\end{itemize}
% 	hence $\C$ is a field.
% \end{proposition}
...
% 	$\C$ with $+$ is an abelian group, with identity $0$, and $\C \backslash \{0\}$ with $\times$ is an abelian group with identity $1$. Furthermore multiplication is distributive over addition.
% \end{proposition}



% The definitions above immediately imply that addition and multiplication are commutative and associative, and that multiplication is distributive over addition. Also, we have defined multiplication in such a way that $i^2 = -1$, consistent with our original definition of the complex numbers.
