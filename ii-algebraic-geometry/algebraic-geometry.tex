...
\begin{example}[A smooth point]
	On $C = \V(Y^2 - X^3 + X)$ take the point $P = (2,\sqrt 6)$. Here $\partial f/\partial Y = 2\sqrt 6 \neq 0$, so $P$ is smooth and there is a genuine tangent line, namely
	$$-11(X-2) + 2\sqrt6\,(Y - \sqrt6) = 0.$$
...
\begin{definition}[Chord-and-Tangent Law]
...
\begin{tikzpicture}[x=1.05cm,y=0.85cm]
	...
	\node[above left] at (-1.65,3.35) {\small $e=1$};
	\node[left] at (-1.7,2.586) {\small $e=1$};
	\node[left] at (-1.7,1.414) {\small $e=1$};
