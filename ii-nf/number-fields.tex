...
\begin{center}
	\begin{tikzpicture}[scale=1.05, every node/.style={font=\small}]
		...
		\draw (d) -- (f); \draw (e) -- (f);
		\draw[decorate, decoration={brace, amplitude=5pt}] (-2.6,-0.1) -- node[left=6pt] {\small inclusion} (-2.6,3.7);
	\end{tikzpicture}
\end{center}
...
\begin{proof}
	...
	using $n - r - s = s$. By Minkowski's theorem \ref{thm:minkowski}(ii)(b) there is a nonzero $\alpha \in \fa$ with $\sigma(\alpha) \in B(t)$.
...
\begin{remark}
	The volume formula is proved by induction on $r+s$. The base cases are $\vol(B_{1,0}(t)) = \vol([-t,t]) = 2t$ and $\vol(B_{0,1}(t)) = \vol\{\abs z\le t/2\} = \pi t^2/4$, both agreeing with the formula. For the inductive step in the real direction,
	\begin{align*}
		\vol(B_{r+1,s}(t)) &= \int_{-t}^{t}\vol\left(B_{r,s}(t-\abs y)\right)\dd y = 2\int_0^t 2^r\left(\frac\pi2\right)^{\!s}\frac{(t-y)^{n}}{n!}\dd y\\
		&= 2^{r+1}\left(\frac\pi2\right)^{\!s}\frac{t^{n+1}}{(n+1)!},
	\end{align*}
	as required, and the complex direction is similar (integrating over a disc in polar coordinates).
\end{remark}
...
\begin{example}[A Cyclotomic Field]
	...
	so $h_K = 1$: the ring $\Z[\zeta_5]$ is a PID. The same computation works for $\Q(\zeta_7)$ and indeed for $\Q(\zeta_p)$ up to $p=19$, but it fails for $p = 23$, where $h = 3$. This is the first \vocab{irregular} prime, and it is exactly the point at which Kummer's proof of Fermat's Last Theorem breaks down (see Section \ref{sec:applications}).
\end{example}
...
Granting this, Theorem \ref{thm:dirichlet} follows at once: we have a short exact sequence
...
\begin{theorem}[Fermat]\label{thm:twosquares}
