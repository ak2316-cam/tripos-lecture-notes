...
\begin{theorem}[First-order condition]\label{sfm-thm:foc}
...
\begin{theorem}\label{thm:spd-from-utility}
	In the setting of Theorem~\ref{sfm-thm:foc}, suppose additionally that $U' > 0$ and $\EE[U'(X_1^*)] < \infty$. Then
	$$
...
\begin{proof}
	Certainly $Z > 0$ and $\EE[Z] = 1/(1+r)$. For the second condition, Theorem~\ref{sfm-thm:foc} gives $\EE[U'(X_1^*)(S_1 - (1+r)S_0)] = 0$, i.e.
	$$
...
\begin{proof}
	Identical to Theorem~\ref{sfm-thm:foc}, differentiating in $\theta$. The second claim follows since $X^* - X$ is of the form $\eta^\top\xi$.
\end{proof}
...
\begin{definition}[Numeraire]
	A \vocab{numeraire} is an adapted process $(M_n)_{0\leq n\leq N}$ with $M_n > 0$ for all $n$, which is the price process of a self-financing portfolio. Prices measured in units of the numeraire are $S_n/M_n$.
\end{definition}
...
\begin{theorem}[Change of numeraire]\label{thm:numeraire}
	Let the market be arbitrage-free with risk-neutral measure $\QQ$ (for the bank-account numeraire), and let $M$ be a numeraire with $M_0 = 1$. Define $\QQ^M \sim \QQ$ by
	$$
	\frac{\dd\QQ^M}{\dd\QQ} = \frac{M_N}{(1+r)^{N}}.
	$$
	Then $\QQ^M$ is a probability measure, and $(S_n/M_n)$ is a $\QQ^M$-martingale. Moreover the time-$n$ price of a claim $Y$ payable at time $N$ is
	$$
	\pi_n = M_n\,\EE_{\QQ^M}\!\left[\frac{Y}{M_N}\ \Big|\ \FF_n\right].
	$$
\end{theorem}

\begin{proof}
	Since $M$ is the value of a self-financing portfolio, $\tilde M_n = M_n/(1+r)^n$ is a $\QQ$-martingale with $\tilde M_0 = 1$, so the proposed density is positive with $\QQ$-expectation $1$ and $\QQ^M$ is an equivalent probability measure. For a positive $\FF_m$-measurable random variable $Z$ the density relation together with the tower property gives the conditional Bayes formula
	$$
	\EE_{\QQ^M}[Z \mid \FF_n] = \frac{\EE_\QQ\big[Z\,\tilde M_m\mid\FF_n\big]}{\tilde M_n}, \qquad n \leq m,
	$$
	because $\EE_\QQ[\dd\QQ^M/\dd\QQ \mid \FF_m] = \tilde M_m$. Taking $Z = S_m/M_m$ and $m = N$,
	$$
	\EE_{\QQ^M}\!\left[\frac{S_m}{M_m}\ \Big|\ \FF_n\right] = \frac{\EE_\QQ[\tilde S_m\mid\FF_n]}{\tilde M_n} = \frac{\tilde S_n}{\tilde M_n} = \frac{S_n}{M_n},
	$$
	using that $\tilde S$ is a $\QQ$-martingale. The pricing formula is the same computation applied to $\pi_m/M_m$, whose $\QQ$-discounted version is a martingale by the second fundamental theorem.
\end{proof}
