...
\begin{example}[$C_m$, completely]
	The irreducible complex representations of $C_m = \gen{g}$ are exactly the $m$ degree-one representations
...
\begin{example}
	The quaternion group $Q_8$ has centre $\{\pm1\}\cong C_2$, which is cyclic, and indeed it does have a faithful irreducible representation -- the two-dimensional one we shall meet later. By contrast the Klein four-group $C_2\times C_2$ is its own centre, which is not cyclic, so the proposition says it has \emph{no} faithful irreducible representation. And indeed all four of its irreducibles have degree $1$, and each of them kills a subgroup of order $2$.

	So already, before we have computed a single character, the theory rules out certain shapes of representation on purely group-theoretic grounds.
\end{example}
...
\end{proposition}
\begin{proof}
	Expand $\Hom_G(\bigoplus_S S^{\oplus n_S}, \bigoplus_T T^{\oplus n_T})$ into a double sum and apply Schur; this gives $\sum_S n_S^2$. If the answer is $1$ then exactly one $n_S$ is $1$ and the rest vanish, so $V \cong S$ is irreducible; the converse is Schur.
\end{proof}
...
\begin{example}
	Take $G = S_4$ acting on $X = \{1,2,3,4\}$. Then $\pi_X$ takes the values $4$, $2$, $0$, $1$ and $0$ on the classes of $e$, $(1\,2)$, $(1\,2)(3\,4)$, $(1\,2\,3)$ and $(1\,2\,3\,4)$ respectively.
\end{example}
...
\begin{proof}
	...
	$$\dim \Hom_G(V,W) = \dim \Hom_\C(V,W)^G = \frac1{|G|}\sum_{g\in G}\chi_{\Hom(V,W)}(g),$$
	and the last expression is $\frac1{|G|}\sum_{g} \overline{\chi_V(g)}\chi_W(g) = \ip{\chi_V}{\chi_W}$.
\end{proof}
...
\end{theorem}
\begin{proof}
	Immediate from the previous theorem and Schur's lemma, which says that $\dim \Hom_G(S_i, S_j)$ is $1$ if $i=j$, and is $0$ otherwise.
\end{proof}
...
It is worth taking this geometrically, because that is really all that is going on. The space $\mathcal C(G)$ is an $r$-dimensional inner product space, and the irreducible characters are an orthonormal basis of it. A character $\chi$ is then just a vector, its multiplicities are its coordinates, and the coordinates are extracted -- as always in an inner product space -- by taking inner products with the basis vectors.
...
\begin{tikzpicture}[scale=1.05]
	...
	\draw[->, thick] (0,0) -- (1,0);
	\draw[->, thick] (0,0) -- (0,1);
	\node[below] at (1,-0.05) {\small $\chi_1$};
	\node[left] at (-0.05,1) {\small $\chi_2$};
	% the character
	\draw[->, very thick, color=blue!70!black] (0,0) -- (2.6,1.7);
	\node[color=blue!70!black, right] at (2.62,1.78) {\small $\chi = n_1\chi_1 + n_2\chi_2 + \cdots$};
	% projections
	\draw[dashed, gray] (2.6,1.7) -- (2.6,0);
	\draw[dashed, gray] (2.6,1.7) -- (0,1.7);
	\fill[color=blue!70!black] (2.6,0) circle (1.3pt);
	\fill[color=blue!70!black] (0,1.7) circle (1.3pt);
	\node[below] at (2.6,-0.32) {\small $n_1 = \ip{\chi_1}{\chi}$};
	\node[left] at (-0.05,1.7) {\small $n_2 = \ip{\chi_2}{\chi}$};
	\node at (1.4,-1.25) {\small $\ip{\chi}{\chi} = \textstyle\sum_i n_i^2$, and $\chi$ is irreducible exactly when this is $1$};
\end{tikzpicture}
\end{center}
\vspace{0.3em}
...
\begin{proof}
	...
	$$\varphi_f\,\rho(h) = \sum_g \overline{f(g)}\rho(gh) = \sum_k \overline{f(kh^{-1})}\rho(k),$$
	$$\rho(h)\,\varphi_f = \sum_g \overline{f(g)}\rho(hg) = \sum_k\overline{f(h^{-1}k)}\rho(k),$$
...
\begin{tikzpicture}[scale=0.86]
	% highlight two rows and two columns, behind the grid
	\fill[blue!20] (0,3) rectangle (5,4);
	\fill[blue!20] (0,2) rectangle (5,3);
	\fill[red!22] (1,0) rectangle (2,4);
	\fill[red!22] (4,0) rectangle (5,4);
	\fill[blue!20!red!22!white] (1,2) rectangle (2,4);
	\fill[blue!20!red!22!white] (4,2) rectangle (5,4);
	% grid
	...
	\draw[decorate, decoration={brace, amplitude=5pt}, blue!60!black] (5.2,4) -- (5.2,2);
	\node[blue!60!black, right] at (5.55,3.5) {\small rows, weighted by $|\ccl(g_j)|$:};
	\node[blue!60!black, right] at (5.55,2.85) {\small $\ip{\chi_i}{\chi_k} = \delta_{ik}$};
...
\end{theorem}
\begin{proof}
	Let $D$ be the diagonal matrix whose entries are $D_{jj} = |\ccl_G(g_j)|/|G| = 1/|\Cent_G(g_j)|$. Then row orthogonality says
	$$\delta_{ik} = \ip{\chi_k}{\chi_i} = \sum_j \frac{\overline{\chi_k(g_j)}\chi_i(g_j)}{|\Cent_G(g_j)|} = \big(X D \overline{X}^{\,T}\big)_{ik},$$
	so $X D \overline X^{\,T} = I$. Thus $X$ is a square invertible matrix, with inverse $X^{-1} = D\overline X^{\,T}$, and therefore $X^{-1}X = I$ as well:
...
\begin{example}[Recovering a row]
	Suppose we know that a group $G$ of order $12$ has four conjugacy classes, of sizes $1,3,4,4$, and that three of its irreducible characters are $(1,1,1,1)$, $(1,1,\omega,\omega^2)$ and $(1,1,\omega^2,\omega)$, where $\omega = e^{2\pi i/3}$. Then the degrees satisfy $1+1+1+d_4^2 = 12$, so $d_4 = 3$; and column orthogonality against the first column gives, for the class of size $3$,
...
\begin{tikzpicture}[scale=1]
	...
	\draw[->, dashed] (Q) -- node[below right=-3pt] {\small $\tilde\rho$} (GL.south);
	\node[right] at (4.4,0.7) {\small (exists iff $N \leq \kernel\rho$)};
\end{tikzpicture}
\end{center}
...
\begin{tikzpicture}[scale=1.9]
	...
	\draw[dashed, color=blue!70!black] ($(P)!-0.30!(Q)$) -- ($(P)!1.18!(Q)$);
	\fill[color=blue!70!black] (P) circle (0.9pt);
	\fill[color=blue!70!black] (Q) circle (0.9pt);
	...
	\node[above left=-2pt] at (D) {\small $4$};
	\node[color=red!75!black, right] at (0.22,1.30) {\scriptsize $3$-fold axis};
	\node[color=blue!70!black, above left=-2pt] at (-0.90,0.42) {\scriptsize $2$-fold axis};
\end{tikzpicture}
\end{center}
...
\begin{tikzpicture}[scale=1.25]
	...
	\coordinate (e) at (1.0,0.5); \coordinate (f) at (3.0,0.5);
	\coordinate (g) at (3.0,2.5); \coordinate (h) at (1.0,2.5);
	...
	\node at (1.5,-0.55) {\small the $4$ space diagonals};
\end{tikzpicture}
\end{center}
...
\begin{tikzpicture}[scale=1.6]
	...
	\foreach \i in {0,...,4} {
		...
		\coordinate (U\i) at ({1.15*cos(\ang)}, {0.60 + 0.42*sin(\ang)});
		\pgfmathsetmacro{\angl}{36 + 72*\i}
		\coordinate (L\i) at ({1.15*cos(\angl)}, {-0.60 + 0.42*sin(\angl)});
	}
	% hidden edges (far side), faint and dashed
	\foreach \p/\q in {U1/U2, L0/L1, T/U1, T/U2, B/L0, B/L1, U1/L0, U1/L1, U2/L1}
		{ \draw[thin, dashed, gray!45] (\p) -- (\q); }
	% visible edges
	\foreach \p/\q in {U0/U1, U2/U3, U3/U4, U4/U0, L1/L2, L2/L3, L3/L4, L4/L0,
		T/U0, T/U3, T/U4, B/L2, B/L3, B/L4,
		U0/L0, U2/L2, U3/L2, U3/L3, U4/L3, U4/L4, U0/L4}
		{ \draw[thin, black!80] (\p) -- (\q); }
...
$$\setlength{\arraycolsep}{5pt}\begin{array}{l|ccccccc}
\text{cycle type} & 1^5 & 2\,1^3 & 2^2 1 & 3\,1^2 & 3\,2 & 4\,1 & 5\\
\text{class size} & 1 & 10 & 15 & 20 & 20 & 30 & 24
\end{array}$$
These sum to $120$, as they must. Since $S_5' = A_5$ there are two linear characters. Let $\pi = (5,3,1,2,0,1,0)$ be the natural permutation character; the standard character is then
...
\begin{proof}
	We have
	$$\ip{\chi|_H}{\chi|_H}_H = \frac{|G|}{|H|}\cdot\frac1{|G|}\sum_{h\in H}|\chi(h)|^2 \; \leq \; |G:H|\cdot\frac1{|G|}\sum_{g\in G}|\chi(g)|^2 = |G:H|,$$
...
\begin{example}[Permutation characters are induced]
	...
	$$\Ind_H^G\mathbf 1(g) = \frac{1}{|H|}\,\#\{x\in G : x^{-1}gx\in H\} = \#\{xH : gxH = xH\},$$
	and the last quantity is $|\operatorname{fix}_X(g)|$ under the identification $X \leftrightarrow G/H$. Frobenius reciprocity then reproves that $\ip{\pi_X}{\mathbf 1_G} = \ip{\mathbf1_H}{\mathbf 1_H} = 1$ for transitive actions.
\end{example}
...
\begin{example}[Building characters of $A_5$ by induction]
	...
	Inducing the trivial character gives the permutation character on $5$ points, namely $(5,1,2,0,0) = \mathbf 1 + \chi_4$.
...
\begin{proof}[Proof sketch]
	...
	$$\ip{\Ind_H^GW}{\Ind_H^GW}_G = \ip{W}{\Res^G_H\Ind_H^GW}_H = \sum_{HgH}\big\langle \Res_{H\cap{}^gH}W,\ \Res_{H\cap{}^gH}({}^gW)\big\rangle.$$
...
\end{theorem}
\begin{proof}
	Write $d = \chi(1)$, and let $C_1,\dots,C_r$ be the conjugacy classes, with representatives $g_1,\dots,g_r$. Then
	...
	using $\ip\chi\chi = 1$ in the first equality. The right hand side is a sum of products of algebraic integers, so is an algebraic integer; the left hand side is rational. Hence $|G|/d\in\mathbb A\cap\Q = \Z$.
\end{proof}
...
$$R_t = \{\sigma : \sigma \text{ preserves each row of } t\}, \qquad C_t = \{\sigma : \sigma\text{ preserves each column of } t\}$$
...
\begin{example}[The irreducibles of $S_5$]
	...
	$$\dim S^{(5)} = 1,\qquad \dim S^{(4,1)} = \tfrac{120}{5\cdot3\cdot2\cdot1\cdot1} = 4,$$
	$$\dim S^{(3,2)} = \tfrac{120}{4\cdot3\cdot1\cdot2\cdot1} = 5,\qquad \dim S^{(3,1,1)} = \tfrac{120}{5\cdot2\cdot1\cdot2\cdot1} = 6,$$
	and the remaining three follow by transposition: $\dim S^{(2,2,1)} = 5$, $\dim S^{(2,1^3)} = 4$ and $\dim S^{(1^5)} = 1$. The squares sum to
...
\begin{remark}
	Equivalently, $\SU(2)$ is the group of unit quaternions. Identify $\HH = \R\gen{1,i,j,k}$ with the set of all matrices $\left(\begin{smallmatrix}a&b\\-\overline b&\overline a\end{smallmatrix}\right)$, with no norm condition imposed; then the quaternion norm is $\det$, and the unit quaternions are exactly $\SU(2)$. This is why $Q_8\leq\SU(2)$, and it is the source of the degree-$2$ representation of $Q_8$ we met earlier.
\end{remark}
...
\begin{tikzpicture}[scale=1.5]
	...
	\draw[gray!55] (1.34,0.35) -- (1.52,0.35);
	\node[color=gray!45!black, right] at (1.55,0.35) {\small conjugacy class $\{\tfrac12\tr = \cos\theta\}$};
...
\end{theorem}
\begin{proof}
	First, a direct computation:
	$$\ip{\chi_n}{\chi_n} = \frac2\pi\int_0^\pi\left|\frac{\sin((n+1)\theta)}{\sin\theta}\right|^2\sin^2\theta\dd\theta = \frac2\pi\int_0^\pi\sin^2\big((n+1)\theta\big)\dd\theta = \frac2\pi\cdot\frac\pi2 = 1,$$
	so $V_n$ is irreducible.
