...
\begin{theorem}[Uniqueness of extension]\label{thm:uniqueness}
...
\begin{theorem}[Lebesgue measure on $\R$]\label{thm:lebesgue}
...
\begin{proof}
	...
	For uniqueness, suppose $\mu, \lambda$ are two Borel measures assigning length $b-a$ to $(a,b]$. For each $n \in \Z$ the measures $A \mapsto \mu(A \cap (n, n+1])$ and $A \mapsto \lambda(A \cap (n,n+1])$ are finite measures of total mass $1$ agreeing on the $\pi$-system $\cA$, so they agree on $\cB$ by Theorem \ref{thm:uniqueness}. Summing over $n \in \Z$ and using countable additivity, $\mu = \lambda$.
\end{proof}
...
\end{proposition}
\begin{proof}
	The map $y \mapsto y - x$ is a homeomorphism of $\R$, so $B \mapsto B + x$ preserves $\cB$. Define $\mu_x(B) = \mu(B+x)$. This is a Borel measure and $\mu_x((a,b]) = \mu((a+x, b+x]) = b-a$. By the uniqueness in Theorem \ref{thm:lebesgue}, $\mu_x = \mu$.
\end{proof}
...
\end{theorem}
\begin{proof}
	The construction of Theorem \ref{thm:lebesgue} goes through verbatim with $b - a$ replaced by $g(b)-g(a)$, right-continuity of $g$ being exactly what is needed to make the shrinking argument work. Alternatively -- and this is the slicker route -- let $f$ be the generalised inverse of Lemma \ref{lem:gen-inverse}. For $z \in \R$,
	$$
...
\end{proposition}
\begin{proof}
	Fix $A_2 \in \cA_2$ and consider the two finite measures $B \mapsto \PP(B \cap A_2)$ and $B \mapsto \PP(B)\PP(A_2)$ on $\sigma(\cA_1)$. They agree on the $\pi$-system $\cA_1$ and have the same total mass $\PP(A_2)$, so by Theorem \ref{thm:uniqueness} they agree on $\sigma(\cA_1)$. Now fix $B \in \sigma(\cA_1)$ and repeat the argument in the second coordinate: the measures $C \mapsto \PP(B \cap C)$ and $C \mapsto \PP(B)\PP(C)$ agree on the $\pi$-system $\cA_2$, hence on $\sigma(\cA_2)$.
\end{proof}
...
\begin{example}[The escaping bump]\label{ex:bump}
...
\end{theorem}
\begin{proof}
	Uniqueness is Theorem \ref{thm:uniqueness}: the rectangles form a $\pi$-system generating $\cE_1\otimes\cE_2$ and containing $E$, and the total mass $\mu_1(E_1)\mu_2(E_2)$ is finite.
...
\end{corollary}
\begin{proof}
	$L^1 \cap L^2$ contains all $f\ind_{B(0,n)}$ for $f \in L^2$, hence is dense; $F_0$ is an isometry there by Theorem \ref{thm:plancherel}; and $L^2$ is complete, so the extension exists and is unique. Preservation of the inner product follows by polarisation.
\end{proof}
...
\begin{lemma}[Maximal ergodic lemma]\label{lem:maximal}
...
\begin{proof}
	...
	Since $\mu$ is finite we have $f_K \in L^1(\mu)$, so Birkhoff gives $S_n(f_K)/n \to \bar f_K$ almost everywhere, and $|S_n(f_K)/n|\leq K$, so that $|\bar f_K|\leq K$. By bounded convergence (Theorem \ref{thm:dct}, with dominating constant $2K$, integrable as $\mu(E)<\infty$),
	$$
