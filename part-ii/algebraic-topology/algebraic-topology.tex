...
\end{corollary}
\begin{proof}
	Suppose $f(x) \neq x$ for all $x$. Define $r(x) \in S^1$ to be the point where the ray from $f(x)$ through $x$ meets $S^1$. This is continuous (we will verify a formula for it in Theorem \ref{at-thm:brouwer}), and if $x \in S^1$ then $r(x) = x$. So $r$ is a retraction $D^2 \to S^1$, contradicting the previous corollary.
\end{proof}
...
\begin{theorem}[Galois Correspondence] \label{at-thm:galois}
...
\begin{remark} \label{rmk:galois-dictionary}
	Under the correspondence of Theorem \ref{at-thm:galois}:
	\begin{enumerate}
...
\begin{theorem}[Homotopy Invariance] \label{at-thm:homotopy-invariance}
...
\end{corollary}
\begin{proof}
	Let $F: |K|\to|L|$ and $G: |L|\to|K|$ with $F\circ G \simeq \id$ and $G \circ F \simeq \id$. Then by Theorems \ref{thm:functorial-homology} and \ref{at-thm:homotopy-invariance}, $F_*G_* = \id$ and $G_*F_* = \id$, so $F_*$ is an isomorphism.
\end{proof}
...
\begin{theorem}[Brouwer Fixed Point Theorem] \label{at-thm:brouwer}
...
\end{proposition}
\begin{proof}
	(i) is functoriality (Theorem \ref{thm:functorial-homology}) and (ii) is homotopy invariance (Theorem \ref{at-thm:homotopy-invariance}). For (iii), a constant map factors through a point, and $H_n(*) = 0$ for $n \geq 1$. For (iv), if $g$ is a homotopy inverse then $\deg g \deg f = \deg\id = 1$ in $\Z$.
\end{proof}
...
\begin{theorem} \label{at-thm:euler}
...
\begin{corollary}
	$\chi(K) = L(\id) = L(\id_{H_\bullet})$, recovering Theorem \ref{at-thm:euler}.
\end{corollary}
