...
Just as regular functions assemble into morphisms, rational functions assemble into rational maps.

\begin{definition}[Rational Map of Affine Varieties]
	Let $V \subseteq \A^n$ be irreducible and $W \subseteq \A^m$ a variety. A \vocab{rational map} $\varphi \colon V \dto W$ is a tuple $\varphi = (f_1,\dots,f_m)$ of rational functions $f_j \in \C(V)$ such that $\varphi(P) \in W$ for every $P$ at which all the $f_j$ are regular. The \vocab{domain} $\dom\varphi$ is the set of such $P$, a dense open subset of $V$.
\end{definition}

A rational map is thus a morphism defined on a dense open subset, and two rational maps agreeing on a dense open subset are equal. If $\varphi(\dom\varphi)$ is dense in $W$ we call $\varphi$ \vocab{dominant}; in that case $\varphi$ pulls back rational functions and gives an inclusion of fields $\varphi^\star \colon \C(W)\hookrightarrow\C(V)$. We will develop all of this properly in the projective setting, where it belongs; for now, here is the example that everyone should have seen.

\begin{example}[Rational parametrisation of the circle]\label{ex:circleparam}
	Let $C = \V(X^2+Y^2-1) \subseteq \A^2$. Define $\varphi \colon C \dto \A^1$ by
	$$\varphi(x,y) = \frac{y}{1+x},$$
	a rational map, regular away from the point $(-1,0)$. Geometrically $\varphi(P)$ is the slope of the line joining $(-1,0)$ to $P$. In the other direction, define $\psi \colon \A^1 \dto C$ by
	$$\psi(t) = \left(\frac{1-t^2}{1+t^2},\ \frac{2t}{1+t^2}\right),$$
	which is regular away from $t = \pm i$, and whose image does lie in $C$ since $(1-t^2)^2 + (2t)^2 = (1+t^2)^2$. A direct substitution gives $\varphi\psi = \mathrm{id}$ and $\psi\varphi = \mathrm{id}$ wherever both sides are defined. So $C$ and $\A^1$ are \emph{birational}, even though $\varphi$ is not defined everywhere: the conic and the line become isomorphic once we delete a point from each.

	Reading the formula for $\psi$ with $t = q/p$ rational and clearing denominators gives $(p^2-q^2)^2 + (2pq)^2 = (p^2+q^2)^2$, which is the classical parametrisation of Pythagorean triples. This is a good advertisement for the subject: a geometric statement about one conic contains an arithmetic statement about infinitely many integers.
\end{example}
...
\begin{tikzpicture}[x=1cm,y=1cm]
	...
	\node[below, color=blue] at (2.0,-0.05) {$P_\infty$};
\end{tikzpicture}
\end{center}
...
The same picture in one dimension higher is the one to carry around for the rest of the course. A point of $\PP^2$ is a line through the origin in $\C^3$; such a line meets the affine plane $\{x_0 = 1\}$ in exactly one point unless it happens to lie in the plane $\{x_0 = 0\}$, in which case it contributes a point of the line at infinity.

\begin{center}
\begin{tikzpicture}[x=1cm,y=1cm]
	% the affine plane {x_0 = 1}
	\draw[thin, color=gray] (-2.6,0.6) -- (1.4,0.0) -- (3.0,1.5) -- (-1.0,2.1) -- cycle;
	\node[color=gray, above] at (-1.75,2.06) {$\{x_0 = 1\}\cong\A^2$};
	% lines through the origin meeting the plane
	\foreach \px/\py in {-1.2/1.05, 0.2/1.05, 1.6/1.05}{
		\draw[thin, color=blue] (0,-1.6) -- ({1.32*\px}, {-1.6 + 1.32*(\py + 1.6)});
		\filldraw[color=blue] (\px,\py) circle (1.4pt);
	}
	% a line in the plane x_0 = 0
	\draw[very thick, color=red] (-2.2,-1.27) -- (2.2,-1.93);
	\node[color=red, right] at (2.25,-1.93) {$x_0 = 0$};
	\filldraw (0,-1.6) circle (1.7pt);
	\node[below left] at (0.02,-1.62) {$\mathbf 0$};
\end{tikzpicture}
\end{center}

The three blue lines are three points of $\A^2 \subseteq \PP^2$; the red line, lying in the plane $x_0 = 0$ and so never meeting $\{x_0=1\}$, is one of the points of $\ell_\infty$.
...
\begin{tikzpicture}[x=1cm,y=1cm]
	...
	\node[above] at (0,2.86) {$P_\infty$};
	\node[color=blue, right] at (1.35,-0.72) {$C \cap U_0$};
\end{tikzpicture}
\end{center}
...
In practice one computes with the definition directly, and in small cases this is easy: the local ring $\Oh_{\A^2,P}$ is obtained from $\C[X,Y]$ by inverting the polynomials not vanishing at $P$, so any factor which is a unit at $P$ may simply be deleted.

\begin{example}\label{ex:intmultcomputations}
	Take $P = (0,0)$ throughout, and write $\Oh = \Oh_{\A^2,P}$.
	\begin{enumerate}[label=(\roman*)]
		\item Let $C = \V(y - x^2)$ be a parabola and $L = \V(y)$ its tangent at $P$. Then
		$$\frac{\Oh}{(y-x^2,\ y)} = \frac{\Oh}{(y,\ x^2)} \cong \frac{\C[x]_{(x)}}{(x^2)},$$
		which has $\C$-basis $\{1,x\}$, so $I_P(C,L) = 2$. The tangency is visible in the algebra as the nilpotent $x$.
		\item Let $L' = \V(y - x)$ instead. Substituting, $(y-x^2,\ y-x) = (y-x,\ x - x^2) = (y-x,\ x)$, since $1-x$ is a unit in $\Oh$. So the quotient is $\C$ and $I_P(C,L') = 1$: the intersection is transverse. Of course $L'$ also meets $C$ at $(1,1)$, and $1 + 1 = 2$ agrees with B\'ezout for a line and a conic.
		\item Let $C$ be the nodal cubic $\V(y^2 - x^2(x+1))$ and $L = \V(y)$, one of the two lines through the node that is \emph{not} a branch tangent. Then
		$$\frac{\Oh}{(y^2-x^3-x^2,\ y)} \cong \frac{\C[x]_{(x)}}{(x^2(x+1))} \cong \frac{\C[x]_{(x)}}{(x^2)},$$
		as $x+1$ is a unit at $P$; so $I_P(C,L) = 2$. The line meets the singular point twice, once on each branch.
		\item With the same $C$ and the branch tangent $L'' = \V(y-x)$, substituting $y = x$ gives $x^2 - x^3 - x^2 = -x^3$, so $I_P(C,L'') = 3$. Indeed a line meets a cubic in three points, so $L''$ meets $C$ nowhere else.
	\end{enumerate}
\end{example}

\begin{center}
\begin{tikzpicture}[x=1.5cm,y=1.0cm]
	\draw[ultra thin, color=lightgray] (-1.3,-0.6) grid[step=0.5] (1.4,1.9);
	\draw[<->] (-1.4,0) -- (1.5,0);
	\draw[<->] (0,-0.7) -- (0,2.0);
	\draw[thick, color=blue, smooth] plot coordinates {(-1.30,1.690) (-1.15,1.323) (-1.00,1.000) (-0.85,0.723) (-0.70,0.490) (-0.55,0.303) (-0.40,0.160) (-0.25,0.063) (-0.10,0.010) (0.00,0.000) (0.10,0.010) (0.25,0.063) (0.40,0.160) (0.55,0.303) (0.70,0.490) (0.85,0.723) (1.00,1.000) (1.15,1.323) (1.30,1.690)};
	\draw[color=red, thick] (-1.35,0) -- (1.45,0);
	\draw[color=DarkGreen, thick] (-0.55,-0.55) -- (1.35,1.35);
	\filldraw (0,0) circle (1.7pt);
	\filldraw (1,1) circle (1.7pt);
	\node[below right] at (0.05,-0.05) {$P$};
	\node[color=red, above left] at (-0.75,0.02) {$L$};
	\node[color=DarkGreen, below right] at (1.05,1.0) {$L'$};
	\node[color=blue, right] at (1.3,1.72) {$C$};
\end{tikzpicture}
\end{center}

The tangent line $L$ meets the parabola $C$ only at $P$, but with multiplicity $2$; the transverse line $L'$ meets it at two distinct points, each with multiplicity $1$. In both cases the total is $2 = 1 \cdot 2$, as B\'ezout demands.
...
\begin{theorem}[Weierstrass Normal Form]\label{ag-thm:weierstrass}
...
\begin{proof}
	...
	The inclusion $\iota \colon W \hookrightarrow V$ gives a surjection $\iota^\star \colon \C[V] \twoheadrightarrow \C[W]$ with kernel $I(W)/I(V)$, which is nonzero since $I(V) \subsetneq I(W)$ by the Nullstellensatz. Choose $x \in \C[V]$ nonzero with $\iota^\star(x) = 0$.

	Suppose for contradiction that $\C[W] \neq \C$, and pick $t \in \C[W]\setminus\C$; choose $y \in \C[V]$ with $\iota^\star(y) = t$. I claim $x$ and $y$ are algebraically independent over $\C$. If not, there is a nonzero $g \in \C[S,T]$ with $g(x,y) = 0$; choose such a $g$ of minimal degree in $S$. Applying $\iota^\star$, $g(0,t) = 0$ in $\C[W]$. Now $t$ is transcendental over $\C$, since $\C$ is algebraically closed and any element of the domain $\C[W]$ that is algebraic over $\C$ must lie in $\C$. Hence $g(0,T)$ is the zero polynomial, i.e.\ $S \mid g$. Then $g = Sg_1$ and $xg_1(x,y) = 0$; as $\C[V]$ is a domain and $x \neq 0$, $g_1(x,y) = 0$, contradicting minimality.

	So $\C(V)$ would contain two algebraically independent elements, which contradicts the fact that $\C(V)$ has transcendence degree $1$. Hence $\C[W] = \C$, so $W$ is a single point.
\end{proof}
...
\begin{tikzpicture}[x=1.05cm,y=0.85cm]
	...
	\node[above right] at (-1.55,3.38) {\small $e=1$};
	\node[above right] at (-1.55,2.62) {\small $e=1$};
	\node[above right] at (-1.55,1.45) {\small $e=1$};
...
\section{Differentials}\label{ag-sec:differentials}
...
\begin{example}[Smooth plane cubics]\label{ag-ex:cubicgenus}
	Let $C = \V(F) \subseteq \PP^2$ with $F = X_0X_2^2 - \prod_{i=1}^3(X_1 - \lambda_iX_0)$ and $\lambda_1,\lambda_2,\lambda_3$ distinct; this is the Weierstrass form of Theorem~\ref{ag-thm:weierstrass}, and it is smooth. In the affine chart $X_0 = 1$ with coordinates $x = X_1/X_0$, $y = X_2/X_0$, the equation is
...
\begin{proof}
	By Theorem~\ref{ag-thm:weierstrass} we may change coordinates into the shape of Example~\ref{ag-ex:cubicgenus}, and the genus is an isomorphism invariant. Since $g(\PP^1) = 0 \neq 1$, the last statement follows.
\end{proof}
...
\begin{theorem}[Riemann--Hurwitz]\label{ag-thm:RH}
...
The next example is the classical one, and it is worth doing slowly because it shows Riemann--Hurwitz being used to count something one would not otherwise know how to count.

\begin{example}[The class of a plane curve]\label{ex:class}
	Let $C \subseteq \PP^2$ be a smooth plane curve of degree $d \geq 2$, so $g = \frac{(d-1)(d-2)}{2}$ by Corollary~\ref{cor:genusdegree}. Fix a point $R \in \PP^2\setminus C$ and let $\pi \colon C \to \PP^1$ be the projection from $R$ of Example~\ref{ex:projpoint}, restricted to $C$; this is a morphism by Corollary~\ref{cor:ratismorph}. The fibre over a point of $\PP^1$ is $C \cap L$ for the corresponding line $L$ through $R$, so $\deg\pi = d$.

	What is the ramification? Choose $R$ general -- in particular not on any tangent line of $C$ that is tangent at two points, and not on any flex tangent. Then $e_P = I_P(C,L) \leq 2$ for the line $L = \overline{RP}$, with $e_P = 2$ exactly when $L$ is tangent to $C$ at $P$. So $\sum_P(e_P-1)$ counts the points of $C$ at which the tangent line passes through $R$. Riemann--Hurwitz gives
	$$(d-1)(d-2) - 2 = d\cdot(-2) + \sum_{P}(e_P-1),$$
	so
	$$\sum_P (e_P - 1) = d^2 - 3d + 2 - 2 + 2d = d(d-1).$$
	Thus \emph{from a general point of $\PP^2$ there are exactly $d(d-1)$ tangent lines to a smooth plane curve of degree $d$}. This number is called the \vocab{class} of $C$. For $d = 2$ it says there are two tangents from an external point to a conic, which is familiar; for $d = 3$ it gives six tangents to a smooth cubic.
\end{example}

\begin{example}
	Projecting from a point $R$ \emph{on} $C$ instead, the line $\overline{RP}$ already meets $C$ at $R$, so the fibres have size $d-1$ and $\deg\pi = d-1$. Riemann--Hurwitz now reads
	$$(d-1)(d-2)-2 = (d-1)(-2) + \sum_P(e_P-1),$$
	giving $\sum_P(e_P-1) = d^2 - d - 2$. For $d = 3$ this is $4$: a smooth cubic, projected from a point on it, is a double cover of $\PP^1$ branched over four points. That is exactly the degree $2$ map $(x,y)\mapsto x$ on $y^2 = p(x)$ with $\deg p = 3$, branched at the three roots of $p$ and at infinity.
\end{example}
...
\subsection{Recovering the Weierstrass Equation}

We defined an elliptic curve twice: once concretely, as a smooth plane cubic with a chosen flex (Theorem~\ref{ag-thm:weierstrass}), and once abstractly, as a genus $1$ curve with a chosen point. It is time to show that the two definitions agree, and the argument is a lovely illustration of how Riemann--Roch is used in practice: one writes down the spaces $L(n[P_0])$ for increasing $n$, watches their dimensions grow by one each time, and waits for the moment when there are more obvious elements than there is room for.

\begin{theorem}\label{thm:g1weier}
	Let $E = (C,P_0)$ be an elliptic curve. Then there exist $x,y \in \C(C)$ and $a,b\in\C$ with $4a^3+27b^2 \neq 0$ such that
	$$\varphi = (1 : x : y) \colon C \longrightarrow \PP^2$$
	is an isomorphism onto the smooth plane cubic $\V(X_0X_2^2 - X_1^3 - aX_0^2X_1 - bX_0^3)$, carrying $P_0$ to the flex $(0:0:1)$.
\end{theorem}

\begin{proof}
	Write $D_n = n[P_0]$. By Corollary~\ref{cor:g1RR}, $\ell(D_n) = n$ for all $n \geq 1$. We build up bases.

	First, $L(D_1) = \C$: a function with at most a simple pole at $P_0$ and no other poles would, if non-constant, give a degree $1$ morphism $C \to \PP^1$ and hence force $g = 0$. Since $\ell(D_2) = 2$, there is $x \in L(D_2)\setminus\C$, and $x$ must have a pole of order exactly $2$ at $P_0$ (order $1$ is excluded as above). Since $\ell(D_3) = 3$, there is $y \in L(D_3)\setminus L(D_2)$, necessarily with a pole of order exactly $3$. So
	$$\nu_{P_0}(x) = -2, \qquad \nu_{P_0}(y) = -3,$$
	and $\{1,x\}$, $\{1,x,y\}$ are bases of $L(D_2), L(D_3)$.

	Now count pole orders. The functions
	$$1,\ x,\ y,\ x^2,\ xy,\ x^3,\ y^2$$
	have poles at $P_0$ of orders $0,2,3,4,5,6,6$ respectively, and no poles elsewhere; so all seven lie in $L(D_6)$. But $\ell(D_6) = 6$, so they satisfy a nontrivial linear relation
	$$\alpha y^2 + \beta xy + \gamma y = \delta x^3 + \varepsilon x^2 + \zeta x + \eta.$$
	Here $\alpha \neq 0$: otherwise the left side lies in $L(D_5)$ while the right has a pole of order exactly $6$ unless $\delta = 0$ too, and then $1,x,y,x^2,xy$ would be dependent, which they are not (their pole orders $0,2,3,4,5$ are distinct, and functions with distinct pole orders at a single point are linearly independent). The same argument gives $\delta \neq 0$. Rescaling $x \mapsto \lambda x$, $y \mapsto \mu y$ suitably, we may take $\alpha = \delta = 1$. Completing the square in $y$ (replacing $y$ by $y - \tfrac12(\beta x + \gamma)$) and then the cube in $x$ removes the remaining terms, leaving
	$$y^2 = x^3 + ax + b.$$

	It remains to identify the map. We have $\deg D_3 = 3 > 2 = 2g$, so by Corollary~\ref{cor:embedbig} the morphism $\varphi_{D_3} = (1:x:y)$ is an embedding. Its image lies in the cubic $\V(F)$, $F = X_0X_2^2 - X_1^3 - aX_0^2X_1 - bX_0^3$, by the relation just derived, and the image is closed (Proposition~\ref{prop:projclosed}) and irreducible of dimension $1$. If $F$ were reducible then the image, being irreducible, would lie in a component of degree $1$ or $2$, hence in a line or a conic; but those are isomorphic to $\PP^1$ by Proposition~\ref{prop:conicisP1}, and $C \cong \varphi(C)$ has genus $1$. So $F$ is irreducible and the image is all of $\V(F)$, giving $C \cong \V(F)$; since $C$ is smooth, so is $\V(F)$, and Theorem~\ref{ag-thm:weierstrass} says this forces $x^3+ax+b$ to have distinct roots, i.e.\ $4a^3+27b^2 \neq 0$.

	Finally, $x$ and $y$ have poles only at $P_0$, so $\varphi(P_0)$ is the unique point of the image with $X_0 = 0$; comparing pole orders, $\varphi(P_0) = (0:0:1)$, which is the flex at infinity of the Weierstrass cubic.
\end{proof}

So the two definitions of `elliptic curve' coincide: a genus $1$ curve with a marked point is the same thing as a plane cubic in Weierstrass form with its flex at infinity, and by Theorem~\ref{thm:elliptic} the group law we defined intrinsically is the chord-and-tangent construction.

\begin{example}
	The recipe is completely explicit. On the curve $y^2 = x^3-x+1$, the space $L(2[P_0])$ is spanned by $1$ and $x$, the space $L(3[P_0])$ by $1,x,y$, and the space $L(4[P_0])$ by $1,x,y,x^2$. To see the last of these directly: a function with a pole of order at most $4$ at infinity and no other poles is a polynomial in $x$ and $y$ of the shape $c_0 + c_1x + c_2y + c_3x^2$, since $\nu_{P_0}(x^iy^j) = -(2i+3j)$ and the only $(i,j)$ with $2i+3j \leq 4$ and $j \leq 1$ (we may always reduce $j$ using $y^2 = x^3-x+1$) are $(0,0),(1,0),(0,1),(2,0)$.
\end{example}
...
refined it to
$$\left\{\begin{array}{c}\text{irreducible varieties up to}\\ \text{birational equivalence}\end{array}\right\} \longleftrightarrow \left\{\begin{array}{c}\text{finitely generated}\\ \text{field extensions of } \C\end{array}\right\},$$
