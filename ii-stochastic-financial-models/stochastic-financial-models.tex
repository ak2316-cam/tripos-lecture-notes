...
\begin{center}
\begin{tikzpicture}[scale=1.0]
	\draw[<->] (-0.3,0) -- (4.6,0) node[below] {\footnotesize $S_N$};
	\draw[<->] (0,-2.4) -- (0,2.4);
	\draw[dashed, color=black!45] (2,0.1) -- (2,-0.35);
	\node[below] at (2,-0.3) {\footnotesize $K$};
	% call
	\draw[very thick, color=blue] (0,0) -- (2,0) -- (4.2,2.2);
	\node[color=blue, right] at (3.55,1.85) {\footnotesize $(S_N-K)^+$};
	% -put
	\draw[very thick, color=examplefg] (0,-2) -- (2,0) -- (4.4,0);
	\node[color=examplefg, right] at (3.3,-0.35) {\footnotesize $-(K-S_N)^+$};
	% sum
	\draw[thick, dotted, color=black] (0,-2) -- (4.4,2.4);
	\node[color=black, above left] at (1.15,-0.65) {\footnotesize $S_N - K$};
\end{tikzpicture}
\end{center}
...
\begin{example}[An American put]\label{ex:american-put}
	...
	At $n=3$: $0,\ 0,\ 2.8,\ 27.1$. At $n=2$:
	$$
	\pi(144) = \max\{0, 0\} = 0,\quad \pi(108) = \max\Big\{0, \tfrac{1.4}{1.05}\Big\} = 1.333,\quad \pi(81) = \max\Big\{19,\ \tfrac{14.95}{1.05}\Big\} = \max\{19, 14.238\} = 19,
	$$
	so at the node $S_2 = 81$ it is optimal to exercise. At $n = 1$:
	$$
	\pi(120) = \max\Big\{0,\ \tfrac{0.667}{1.05}\Big\} = 0.635,\qquad \pi(90) = \max\Big\{10,\ \tfrac{10.167}{1.05}\Big\} = \max\{10,\ 9.683\} = 10,
	$$
	so exercise at $S_1 = 90$ too. Finally $\pi_0 = \max\{0, \tfrac{1}{1.05}\cdot\tfrac12(0.635+10)\} = 5.064$.
...
\begin{tikzpicture}[scale=1.0]
	...
	\node[color=examplefg, font=\footnotesize, right] at (5.85,-1.9) {exercise};
	\node[color=examplefg, font=\footnotesize, right] at (3.15,-0.95) {exercise};
	\foreach \x/\t in {0/0,2.7/1,5.4/2,8.1/3}{
		\node[font=\footnotesize, color=black!60] at (\x,-3.55) {$n=\t$};
	}
\end{tikzpicture}
