...
By construction $(e^{-rt}\pi_t)$ is a $\QQ$-martingale, so the augmented market has no arbitrage; and one can show (though we shall not) that the Black--Scholes model is complete, so this is the \emph{only} arbitrage-free price.
...
\begin{proof}
	...
	$$
	Hence $\Delta = \Phi(d_1)$, and differentiating once more gives $\Gamma$. The remaining Greeks follow similarly, using the same identity to cancel the terms coming from differentiating $d_1$ and $d_2$.
\end{proof}
...
\begin{proof}
	Both are immediate from the representation $V(t,s) = e^{-r\tau}\EE\big[g\big(se^{(r-\sigma^2/2)\tau+\sigma\sqrt\tau Z}\big)\big]$: for each fixed value of $Z$ the integrand is an increasing (respectively convex) function of $s$ whenever $g$ is, and both properties survive taking expectations.
\end{proof}
